...
class Syslog extends Transport {
  ...
  log(info, callback) {
    ...
    let syslogMsg = this.producer.produce({
      severity: info[LEVEL],
      host: this.localhost,
      date: new Date(),
    ...
    this.connect((err) => {
      ...
      const sendDgram = () => {
        console.log("Send dgram", syslogMsg);
  ...
  close() {
    ...
    const _close = () => {
      if (attempt >= max || (this.queue.length === 0 && this.inFlight <= 0)) {
        if (this.socket) {
          //console.log('Calling on socket', this.socket, this.socket.destroy);
          this.socket.destroy();
        }

        this.emit('closed', this.socket);
      } else {
        //console.log(`Attempt ${attempt} of ${max} - Syslog`);
        attempt++;
  ...
  setupEvents() {
    ...
    //
    const onError = (logErr) => {
      //console.log('onError triggered', logErr);
      if (logErr) {
